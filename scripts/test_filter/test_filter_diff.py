...
if OSCILLOSCOPE == "" or ARBWAVE30 == "":
    ...
    # 配置基础参数
    arbwave30.write("SOURce1:FREQuency 10000")
    ...
    oscilloscope.write(":CHAN2:SWIT ON")
        #设置耦合
    oscilloscope.write(":CHAN1:COUP DC")
    oscilloscope.write(":CHAN2:COUP DC")
        # 设置带宽限制
    oscilloscope.write(":CHAN1:BWL FULL")
    oscilloscope.write(":CHAN2:BWL FULL")
        # 设置垂直标尺
    oscilloscope.write(":CHAN1:SCAL 0.1") 
    oscilloscope.write(":CHAN2:SCAL 0.1")
        # 设置垂直位置
    ...
    # 配置数学通道
    oscilloscope.write(":FUNC1 ON")
    oscilloscope.write(":FUNC1:OPER SUBT")
    oscilloscope.write(":FUNC1:SOUR1 C1")
    oscilloscope.write(":FUNC1:SOUR2 C2")
    oscilloscope.write(":FUNC1:SCAL 0.2")
    ...
    oscilloscope.write(":MEAS:ADV:P1:SOUR1 F1")
    ...
    for exponent in range(1, 8):  # 指数项
        ...
        for i in range(1, 10):  # 系数项
            if exponent != 7 or i < 7:  
                ...
                vpp_list.append(vpp)
                db_list.append(20 * math.log10(vpp / 2  ))  # 转换为dB
    print("测试完成，测试结果如下：")
    ...
    ax1.semilogx(x, y, label="Filter Response")
